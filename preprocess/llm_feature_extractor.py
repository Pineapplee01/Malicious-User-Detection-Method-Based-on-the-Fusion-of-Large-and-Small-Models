...
class LLMFeatureExtractor:
    ...
    def extract_llm_enhanced_features(self, user_description, tweet_text=None):
        ...
        if user_description and len(user_description) > 0:
            ...
            # 调用_generate_description_prompt微调模型
            ...
            # 提取LLM分析结果的特征
            des_features_raw = torch.Tensor(self.base_feature_extractor(llm_analysis))
            des_features = des_features_raw.mean(dim=1).squeeze()
        else:
            des_features = torch.zeros(768)
        ...
        if tweet_text and len(tweet_text) > 0:
            ...
            # 提取推文分析结果的特征 llm features -> RoBERTa Transfer -> 768-dim
        ...
        # 4. 融合所有特征
        # 简单拼接特征向量
        enhanced_features = torch.cat([base_features, des_features, tweet_features], dim=0)
