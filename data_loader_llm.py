...
def load_data_llm(args):
    ...
    if os.path.exists(user_llm_file) and getattr(args,"ablate_llm", True):
        user_llm_features = torch.load(user_llm_file, map_location='cpu')
    else:
        print(f"警告: 未使用用户LLM特征 {user_llm_file}，使用零填充")
    ...
    if args.lst:
        ...
        if os.path.exists(list_llm_file) and getattr(args,"ablate_llm", True):
            list_llm_features = torch.load(list_llm_file, map_location='cpu')
        else:
            print(f"警告: 未使用列表LLM特征 {list_llm_file}，使用零填充")
